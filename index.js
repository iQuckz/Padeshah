...
const DEFAULT_RANKS = [
  { id: 1, title: "سرباز سوم", emoji: "🥉", min_messages: 0, sort_order: 10 },
  { id: 2, title: "سرباز دوم", emoji: "🥈", min_messages: 50, sort_order: 20 },
  { id: 3, title: "سرباز اول", emoji: "🥇", min_messages: 150, sort_order: 30 },
  { id: 4, title: "سرلشکر", emoji: "🎖️", min_messages: 1000, sort_order: 40, group_choice_key: "high1" },
  { id: 5, title: "سپهبد", emoji: "🎖️", min_messages: 1000, sort_order: 50, group_choice_key: "high1" },
  { id: 6, title: "ارتشبد", emoji: "👑", min_messages: 2500, sort_order: 60 },
  { id: 7, title: "پادشاه", emoji: "🔱", min_messages: 5000, sort_order: 70 }
];
...
async function handleChartCommand(botToken, chatId, messageId, env, currentRanks, settings, isEdit = false) {
  ...
  if (topUsers.results && topUsers.results.length > 0) {
    topUsers.results.forEach((user, idx) => {
      ...
      chartText += `${medal} <b>رتبه ${toPersianDigits(idx + 1)}:</b> <code>${user.first_name || "کاربر"}</code>\n↳ درجه: ${userRank ? userRank.emoji : "🎖️"} <b>${userRank ? userRank.title : "سرباز"}</b> | پیام‌ها: <code>${toPersianDigits(user.message_count)}</code>\n\n`;
    });
  } else {
...
async function handleCheckPermsCommand(botToken, chatId, messageId, settings, env) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      text += `❌ خطا در استعلام: <code>${res.description}</code>`;
    }
  } catch (err) {
...
async function handleAdminPromote(botToken, chatId, targetUserId, targetFirstName, env, currentRanks, settings) {
  ...
  await sendMessage(botToken, chatId, `🎖️ <b>| ارتقای مستقیم شاهنشاه</b>\n━━━━━━━━━━━━━━━━━━\n🎉 جنگجو <b>${targetFirstName}</b> به درجه <b>${nextRank.emoji} ${nextRank.title}</b> منصوب شد!`);
  return new Response("OK", { status: 200 });
}
